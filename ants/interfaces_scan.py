...
# returns the last ether interface that is up along with the external wireless interface and internal wireless interface if it exists
def get_devices(s):
	arr = s.split("*-network")
	dev_wlan_internal = None
	for a in arr:
		t = get_type(a)
		if t:
			if "Ethernet" in t:
...
def interfaces_scan():
	...
	if dev_wlan_internal is not None:	
		print("THE LOGICAL NAME OF THE INTERNAL WIRELESS INTERFACE IS:", dev_wlan_internal.name)
	else:
		print("THIS MACHINE HAS NO INTERNAL WIRELESS INTERFACE")
		dev_wlan_internal = Dev(None, None, None, None)
